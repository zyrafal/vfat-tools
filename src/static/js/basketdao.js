  ...
  consoleInit(main)
    ...
    async function main() {
      ...
      await loadBaskChefContract(App, BASK_CHEF0, BASK_CHEF_ADDR0, BASK_CHEF_ABI,
        "BASK", "SUSHI", "baskToken", "sushiToken", "pendingBaskRewards", "pendingSushiRewards", prices);
...
async function loadBaskChefContract(App, chef, chefAddress, chefAbi, rewardTokenTicker0, rewardTokenTicker1,
    rewardTokenFunction0, rewardTokenFunction1, pendingRewardsFunction0, pendingRewardsFunction1, prices,
    extraPrices, showAll) {
  ...
  const rewardTokenAddresses = [rewardBaskTokenAddress, rewardSushiTokenAddress];
  await getNewPricesAndTokens(App, tokens, prices, rewardTokenAddresses, chefAddress);
  ...
  let baskRewardsPerWeek = 0; //cant receive the rewards per week from the contract
  ...
  const slpContractAddress = poolToken.address;
  const slpContractAbi = [
    {
      "inputs" : [
  ...
  const slpContract = new ethers.Contract(slpContractAddress, slpContractAbi, App.provider);
  const totalStakedSlps = await slpContract.balanceOf(sushiMasterChefAddress) / 10 ** poolToken.decimals;
  poolToken.staked = totalStakedSlps;
